 ...
 * 1) 메소드는 라턴값이 있어야 하는데 없으면 void 기술
 ...
 * - 메소드가 받는 인수 (*파라미터) 와 데이터타입은 메소드를 호출하는 쪽과 일치해야함
 * - Method Overloading(중복정의), Overriding(재정의) 기술로 확장
 * - 메소드가 리턴하는 값과 리턴되는 값의 테이터 타입은 일치해야 함
 * 
 * 메소드의 호출 방법 
 * - 참조 변수, 메소드 이름(파라미터, 파라미터....);
 * 
 * 메소드 종류
 * -static 메소드 = 정적 메소드, 클래스 메소드, static 이 붙은 메소드
 * 	인스턴스 멤버를 사용할 수 없다.
 * 	인스턴스를 사용하지 않아도 사용가능
 * 	고정이기 때문에 오버로딩 오버라이딩 못한다 상속개념이 없어지는 순간 
 * 
 * -non-static 메소드 = 일반 메소드, 인스턴스 메소드, static이 안 붙어있다.
